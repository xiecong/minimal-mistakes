...
The logistic function can be written as: $Y = p(1|x_1,x_2, ...,x_p) = \frac{1}{1+e^{-(\beta_0+\beta X)}}$, where $p$ is the number of independent variables, $n$ is number of the instances.
...
$log(L) = \sum_{i}{n} (1 - y^{(i)}) log(1 - p(1|x_1^{(i)},x_2^{(i)}, ...,x_p^{(i)}))+y^{(i)} log(p(1|x_1^{(i)},x_2^{(i)}, ...,x_p^{(i)}))$
